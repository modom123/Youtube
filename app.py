...
[CTA] ({duration-3}-{duration}s): ..."""}]
        ...
        # ── Step 5b: (removed) Background music bed.
        # The bed was generated by ElevenLabs Sound-Generation, which is an
        # SFX/ambience generator, not a music generator. For a "background
        # music" prompt it returned a broadband noise texture that, looped
        # under the whole voiceover, came through as constant static and made
        # the narration hard to hear. Until a real music provider (Suno /
        # Replicate) is wired in, ship the clean ElevenLabs voiceover on its
        # own — no bed, no music-only intro splash.
        SPLASH_DUR = 2.5
        splash_audio_path = None  # no music bed → no intro-splash audio below
