...
[CTA] ({duration-3}-{duration}s): ..."""}]
        ...
        # ── Step 5b: Background music bed via the ElevenLabs *Music* API
        # (real instrumental music), ducked under the voiceover, plus a short
        # music-only brand intro splash before the voice starts. The old bed
        # used ElevenLabs Sound-Generation (an SFX/ambience generator) which
        # returned broadband noise/static for a "music" prompt; the Music API
        # generates a coherent full-length instrumental instead. Entirely
        # optional polish: any failure just skips it, never blocks the
        # commercial from completing.
        step("Composing background music...", 60)
        ...
        try:
            ...
            bed_path = generate_background_bed(
                f"{style} instrumental background music for a {duration}-second "
                "product commercial, no vocals, subtle and unobtrusive, "
                "steady groove that sits behind a voiceover",
            ...
            if bed_path:
                ...
                if body_bed:
                    ...
                    # Low bed level + sidechain ducking keep the voice on top.
                    if mix_voice_and_music(Path(audio_path), body_bed, mixed_path, music_vol=0.12):
