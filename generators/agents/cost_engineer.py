...
2. PROTECT priority-2 assets — never downgrade them to free_pexels_api or free_stock_internal. Priority-2 assets represent the second most important sections; stripping AI video from them destroys quality where it matters.
   - In critical_save, priority-2 Higgsfield assets may be downgraded to wan2_6 (cheapest Higgsfield model) to save credits — but they MUST remain Higgsfield, never Pexels.
   - In warning, keep priority-2 at lower-cost models (minimax_hailuo or wan2_6 max).
3. In critical_save: swap ALL priority-3 assets to free_pexels_api.
...
- Any higgsfield_* priority-3 → free_pexels_api = saves all credits for that asset
- NEVER swap priority-1 or priority-2 → free_pexels_api
